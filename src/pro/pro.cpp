...
// strings (emitted verbatim) and the response structs expose payment/user status as small enums
// that we surface to JS as their raw integer values (they line up 1:1 with the desktop numeric
// enums), so the only enum->string mapping still needed is for the pro-features result status.
